...
/**
 * Scheduled task to remove orphaned local_mcpbridge_token_scope rows.
 *
...
/**
 * Cleanup task for local_mcpbridge_token_scope.
 */
class cleanup_orphaned_scope extends scheduled_task {
    ...
    public function get_name() {
        return get_string('task_cleanup_orphaned_scope', 'local_mcpbridge');
    }
    ...
    public function execute() {
        ...
        mtrace('local_mcpbridge: removing orphaned token_scope rows...');

        $sql = "DELETE FROM {local_mcpbridge_token_scope}
                 WHERE token NOT IN (SELECT token FROM {external_tokens})";
        $DB->execute($sql);

        mtrace('local_mcpbridge: orphaned token_scope cleanup complete.');
    }
}
